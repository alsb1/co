...
void print(Element* a, int n) {
    for(int i = 0; i < n; i++)
        output << "(" << double(a[i].x)/10.0 << " " << double(a[i].y)/10.0 << " " << double(a[i].z)/10.0 << ") ";
}
...
void terne_counting() {
    ...
    input >> str;
    a[0].y = atoi(convert(str).c_str());
    input >> str;
    a[0].z = atoi(convert(str.substr(0, str.length()-1)).c_str());
    ...
    for(int i = 1; i < n; i++) {
        ...
        input >> str;
        a[i].y = atoi(convert(str).c_str());
        input >> str;
        a[i].z = atoi(convert(str.substr(0, str.length()-1)).c_str());
